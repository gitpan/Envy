...
sub GO() {
	...
	if (@ARGV == 1 and $ARGV[0] eq 'all') {
	    for my $phase (1..2) {
		for my $m ($db->to_unload($phase)) {
		    $db->do_envy($m, 1) if !$db->is_unloaded($m);
		}
	    }
	} else {
...
2] EXAMPLE 'java.env'
   ...
   TOP=\$ENVY_BASE                    # Real path to envy without etc/envy/.
   TOP=\$ENVY_LINKBASE                # Path to envy without etc/envy/.
